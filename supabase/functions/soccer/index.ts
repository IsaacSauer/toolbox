...
// CORS) and shouldn't hold the multi-call prediction logic, so this function
// does both. Each user brings their OWN football-data.org API token — the
// browser stores it in that user's RLS-protected utility_configs row and
// passes it here per request via the `x-fd-token` header. We never persist it.
//
// The free tier covers 12 major competitions with current-season schedules,
// results and head-to-head (10 req/min, no daily cap), but NOT line-ups, so
// predictions are built from recent goals + head-to-head.
//
// Actions (?action=):
...
//
// Deploy with: npx supabase functions deploy soccer

const API_BASE = 'https://api.football-data.org/v4'

const CORS_HEADERS = {
  'Access-Control-Allow-Origin': '*',
  'Access-Control-Allow-Headers': 'authorization, x-client-info, apikey, content-type, x-fd-token',
...
const ts = (m: Json) => new Date(m?.utcDate).getTime()
const isFinished = (m: Json) => m?.status === 'FINISHED'

// Points a team earned in one finished match (3/1/0), or null if unfinished.
function pointsFor(m: Json, teamId: number): number | null {
...
function goalsAvg(matches: Json[], teamId: number): { gf: number | null; ga: number | null; n: number } {
  let gf = 0
  let ga = 0
  let n = 0
...
function h2hOutcome(meetings: Json[], homeId: number): { outcome: Outcome; count: number } {
  let h = 0
  let d = 0
  let a = 0
...
function blend(a: Outcome, wa: number, b: Outcome, wb: number): Outcome {
  return normalize({
...
function recentBefore(matches: Json[], cutoffTs: number): Json[] {
  return matches
...
async function handleFixtures(apiGet: ReturnType<typeof makeClient>, p: URLSearchParams) {
  ...
  const upcoming: Json[] = scheduled.matches ?? []
  if (upcoming.length) {
  ...
  const recent: Json[] = (finished.matches ?? []).sort((a: Json, b: Json) => ts(b) - ts(a)).slice(0, 20)
  return { seasonFinished: true, fixtures: recent.map(mapMatch) }
}

async function handleTeams(apiGet: ReturnType<typeof makeClient>, p: URLSearchParams) {
  const competition = p.get('competition')
  if (!competition) throw new Error('Missing competition')
  ...
  return (data.teams ?? [])
    ...
    .sort((a: Json, b: Json) => a.name.localeCompare(b.name))
}

async function handlePredict(apiGet: ReturnType<typeof makeClient>, p: URLSearchParams) {
  ...
  const h2hMeetings = (h2h.matches ?? []).filter((x: Json) => isFinished(x) && ts(x) < kickoff)
...
function teamName(matches: Json[], teamId: number): string {
  for (const m of matches) {
...
const involves = (m: Json, teamId: number) =>
  m?.homeTeam?.id === teamId || m?.awayTeam?.id === teamId

async function handleMatchup(apiGet: ReturnType<typeof makeClient>, p: URLSearchParams) {
  ...
  let homeMatches: Json[]
  let awayMatches: Json[]
  if (competition) {
    const comp = await apiGet(`/competitions/${competition}/matches`)
    const all: Json[] = (comp.matches ?? []).filter(isFinished)
